...
/**
 * A creator handle doubles as the member's public username, so it is the one
 * field a member cannot change casually — it appears on every listing they
 * publish. Reserved words keep the namespace clear of route collisions.
 */
const reservedHandles = new Set([
  "admin", "api", "account", "explore", "products", "dashboard", "settings",
  "new", "edit", "login", "logout", "register", "termspace", "support", "help",
]);
...
export const creatorProfileSchema = z.object({
  ...
  handle: z
    ...
    .refine((value) => !reservedHandles.has(value), "That username is reserved"),
  bio: z.string().trim().min(10).max(400),
});

/** Handle changes are deliberately excluded: the username is claimed once. */
export const updateCreatorProfileSchema = creatorProfileSchema.omit({ handle: true }).partial();

const productTypes = [
  "Prompt", "Prompt pack", "Skill", "Agent", "Workflow", "MCP server", "AI tool", "Developer utility",
] as const;
const platforms = ["ChatGPT", "Claude", "Codex", "Cursor", "VS Code", "Gemini", "API"] as const;
const models = ["GPT-5", "Claude 4", "Gemini 2.5", "Model agnostic"] as const;

/**
 * A community submission. Price is absent by design — there is no payment
 * provider wired up, so everything a member publishes is free. Trust flags
 * (`verified`, `featured`, `trending`) are never client-supplied.
 */
export const createCommunityProductSchema = z.object({
  name: z.string().trim().min(3).max(120),
  slug: z.string().trim().toLowerCase().regex(slugPattern).min(3).max(120),
  type: z.enum(productTypes),
  category: z.string().trim().min(2).max(80),
  outcome: z.string().trim().min(10).max(180),
  description: z.string().trim().min(20).max(4000),
  platforms: z.array(z.enum(platforms)).min(1).max(7),
  models: z.array(z.enum(models)).min(1).max(4),
  tags: z.array(z.string().trim().min(1).max(30)).max(8).optional().default([]),
  version: z.string().trim().max(20).optional().default("1.0.0"),
});

export const updateCommunityProductSchema = createCommunityProductSchema
  .omit({ slug: true })
  .partial()
