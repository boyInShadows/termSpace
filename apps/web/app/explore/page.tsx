...
export default async function ExplorePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const filters: ProductFilters = { q: first(raw.q), type: first(raw.type), category: first(raw.category), platform: first(raw.platform), sort: first(raw.sort) ?? "featured", page: 1, limit: 12 };
